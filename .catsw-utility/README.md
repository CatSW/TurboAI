...
Guida rapida e operativa per l'utente all'uso degli strumenti di automazione e governance presenti nella directory `.catsw-utility`.
...
*Lettura metodologica consigliata:* [TurboAI Documento TDM](./.catsw-utility/docs/TurboAI.md)

---

## Avvio di TurboAI

Per avviare una sessione TurboAI, eseguire `aaa-startup-llm-session.cmd` dalla cartella `.catsw-utility`.
...
1. Aprire PowerShell nella cartella `.catsw-utility`.
...
Eseguire il comando nella sessione PowerShell in .catsw-utility e incollare il contenuto degli Appunti nella chat LLM.
...
Output generato: Viene creato automaticamente il file context-out-<descrizione>.md dentro la cartella .catsw-utility.
...
Incollare il blocco nella sessione PowerShell aperta in .catsw-utility.
...
Applicazione automatica: from-llm-watcher rileva l'artefatto e invoca process-from-llm, che estrae lo ZIP nella root del repository o esegue lo script di verifica in .catsw-utility/temp/.
...
Generazione del Bundle: Il watcher esegue lo script che crea il file context-out-<descrizione>.md dentro la cartella .catsw-utility.

Impacchettamento ZIP: Dalla cartella .catsw-utility, eseguire:

```DOS
genera-zip.cmd
```
Lo script produce il file .catsw-utility\output\FromLlm-<descrizione>.zip.
...
## 7. Utility Script in .catsw-utility
...
1. Copiare o generare un file context-out-<descrizione>.md (in chiaro BundleFormatVersion 3 o Base64) dentro .catsw-utility\.
2. Eseguire genera-zip.cmd.
3. Recuperare lo ZIP creato in .catsw-utility\output\FromLlm-<descrizione>.zip.
