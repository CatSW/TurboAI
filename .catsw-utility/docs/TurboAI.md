...
[ Scrittura di .catsw-utility/context-out-<descrizione>.md ] (BundleFormatVersion 3)
       │
       ▼ (Esecuzione di .catsw-utility/genera-zip.cmd)
[ Creazione di .catsw-utility/output/FromLlm-<descrizione>.zip ]
       │
...
1. **Emissione dello Script Generator:** L'LLM sul Canale C genera un unico blocco di codice Python contenente la struttura dati completa del contesto (`# BundleFormatVersion: 3`), completa di tag `<<<FILE path="..." bytes="..." sha256="...">>>` e di un eventuale script operativo in `.catsw-utility/temp/FromLlm-<descrizione>.py`.
...
3. **Generazione del Context-Out:** L'esecuzione dello script scrive il file Markdown bundle **rigorosamente dentro la cartella `.catsw-utility/`** con nome `context-out-<descrizione>.md`.
4. **Impacchettamento ZIP Localizzato:** L'utente o il daemon esegue `.catsw-utility/genera-zip.cmd`. Il tool legge `context-out-<descrizione>.md` e genera il file `.catsw-utility/output/FromLlm-<descrizione>.zip`.
...
sequenceDiagram
    ...
    rect rgb(255, 245, 238)
        ...
        FS->>FS: Esecuzione script -> scrive .catsw-utility/context-out-<desc>.md
        U->>FS: Esegue genera-zip.cmd in .catsw-utility
        FS-->>FS: Produce .catsw-utility/output/FromLlm-<desc>.zip
    end

    rect rgb(245, 245, 245)
        note over W, FS: Elaborazione Automatica Daemon & Unbundler
        W->>FS: Rileva file stabile (Downloads o .catsw-utility/output)
        W->>O: Lancia process-from-llm
        alt File è context-request-*.md
            O->>CB: Esegue ContextBundler.exe
            CB-->>FS: Produce context-out-*.md in .catsw-utility
        ...
        O->>FS: Archivia artefatto in .catsw-utility/history/ (con timestamp)
    end
...
flowchart TD
    ...
    subgraph UTIL[".catsw-utility/ (Root Strumentale)"]
...
- **Ubicazione Metadati:** Tutti i file temporanei e i contesti agentici vivono esclusivamente sotto `.ai-context/` e `.catsw-utility/`.
...
└── testdir/                 # Root fisica reale del test
    ...
    ├── .catsw-utility/      # Copia isolata degli strumenti TurboAI
...
- **Separazione Netta tra Framework e Target Code:** `TurboAI` costituisce un'infrastruttura di supporto isolata e generica. L'applicazione della suite su progetti commerciali non altera la licenza dei progetti target, poiché tutti i metadati di governance risiedono nelle cartelle `.ai-context/` e `.catsw-utility/`.
